...
def get_datasets():
    ...
    # soybean = fetch_ucirepo(id=90)
    # healthy_aging = fetch_ucirepo(id=936)
...
def calculate_rule_overlap(results):
    for dataset in results:
        for algorithm in results[dataset]:
            print(results[dataset][algorithm]["rules"])
...
if __name__ == '__main__':
    ...
    for dataset in dataset_list:
        ...
        # optimization_based_arm_input = pd.DataFrame(dataset.data.features, columns=list(dataset.data.features.columns))
    ...
    with open('results.csv', 'w', newline='') as csvfile:
        ...
        for dataset in results:
            ...
            for algorithm in results[dataset]:
                if len(results[dataset][algorithm]['stats']) > 0:
